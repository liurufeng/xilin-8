 ...
 ************************************************************/
var $ = jQuery;
var tb_pathToImage = "/admin/images/loadinglit.gif";
...
function tb_show(caption, url, refParent) {
  ...
  if (!params['modal']) {
    document.onkeyup = function (e) {
      kc = (e == null ? event.keyCode : e.which);
      if (kc == 27) {
        tb_remove();
      }
    };
...
function tb_remove() {
  ...
  if (isparent) {
    ...
    $("#TB_window", parent.document).fadeOut("fast", function () {
      $('#TB_window,#TB_overlay,#TB_HideSelect', parent.document).trigger("unload").unbind().remove();
    });

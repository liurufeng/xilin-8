...
function SeePicNew(f, imgdid, frname, hpos, acname)
{
  ...
  nForm.action = HTTPURL+"info/archives_do?type="+imgdid;
  nForm.target = frname;
  ...
  newobj.innerHTML = '<img src="/admin/images/loadinglit.gif" width="16" height="16" alit="" />上传中....';
  }
newobj.style.display = 'block';
//提交后还原form的action等参数
nForm.action = acname;
...
function SeePicsNew(f, imgdid, frname, hpos, acname)
{
  var newobj = null;
  if(f.value=='') return ;
  vImg = $Obj(imgdid);
...
function LoadQuickDiv(e, surl, oname, w, h)
{
	...
	newobj.innerHTML = '<div style="margin-top:10px;margin-left:10px;"><img src="/admin/images/loadinglit.gif" /> Loading...</div>';
newobj.style.display = 'block';
var myajax = new DedeAjax(newobj);
myajax.SendGet(surl);
fixupPos = false;
...
{
  fixupPos = true;
  LoadQuickDiv(e, 'archives_do?dopost=getCatMap&targetid='+targetId+'&channelid='+cid+'&oldvalue='+oldvalue+'&rnd='+Math.random(), 'getCatMap', '700px', '500px');
  ChangeFullDiv('show');
  }
//显示下级信息Map地图
function ShowinfoMap(e, obj, cid, targetId, oldvalue)
{

  fixupPos = true;
  LoadQuickDiv(e, HTTPURL+'info/doaction?dopost=getinfoMap&targetid='+targetId+'&channelid='+cid+'&oldvalue='+oldvalue+'&rnd='+Math.random(), 'getCatMap', '700px', '500px');
  ChangeFullDiv('show');
  }

...
class CI_Imap {
  ...
  public function imap_connect($config = [])
  {
    ...
    $this->mailbox = $config['host'];
    $this->stream = imap_open($config['host'], $config['user'], $config['pass']);
  ...
  protected function get_trash()
  {
    foreach ($this->get_folders() as $folder)
    {
      if (strtolower($folder) === 'trash' || strtolower($folder) === 'papierkorb' || $folder === '[Gmail]/Trash')
      {
        return $folder;
      }
    }
  ...
  protected function get_sent()
  {
    foreach ($this->get_folders() as $folder)
    {
      if (strtolower($folder) === 'sent' || strtolower($folder) === 'gesendet' || $folder === '[Gmail]/Sent Mail')
      {
        return $folder;
      }
    }
  ...
  protected function format_message($id = 0, $with_body = TRUE, $embed_images = TRUE)
  {
    ...
    if (isset($email['attachments']) && (count(@$email['attachments']) > 0) && (@$email['html'] == TRUE) && ($embed_images == TRUE))
    {
